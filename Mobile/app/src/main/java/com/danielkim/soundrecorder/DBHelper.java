...
public class DBHelper extends SQLiteOpenHelper {
    ...
    private static final String SQL_CREATE_ENTRIES =
            "CREATE TABLE " + DBHelperItem.TABLE_NAME + " (" +
                    ...
                    DBHelperItem.COLUMN_NAME_IS_FAVOURITE + " TEXT_TYPE " + COMMA_SEP +
                    DBHelperItem.COLUMN_NAME_IMAGE_PATH + TEXT_TYPE + ")";
    ...
    public RecordingItem getItemAt(int position, String order, OrderBy orderBy) {
        ...
        String[] projection = {
                ...
                DBHelperItem.COLUMN_NAME_IS_FAVOURITE,
                DBHelperItem.COLUMN_NAME_IMAGE_PATH
        };
        Cursor c = db.query(DBHelperItem.TABLE_NAME, projection, null, null, null, null, null);
        if(orderBy == TIME){
            c = db.query(DBHelperItem.TABLE_NAME, projection, null, null, null, null, DBHelperItem.COLUMN_NAME_TIME_ADDED + order);
        } if(orderBy == ALPHABETICAL){
            c = db.query(DBHelperItem.TABLE_NAME, projection, null, null, null, null, DBHelperItem.COLUMN_NAME_RECORDING_NAME + order);
        } if(orderBy == DURATION){
            c = db.query(DBHelperItem.TABLE_NAME, projection, null, null, null, null, DBHelperItem.COLUMN_NAME_RECORDING_LENGTH + order);
        } if(orderBy == THUMBNAIL) {
            c = db.query(DBHelperItem.TABLE_NAME, projection, null, null, null, null, DBHelperItem.COLUMN_NAME_IMAGE_PATH + order);
        } if(orderBy == FAVOURITE) {
            c = db.query(DBHelperItem.TABLE_NAME, projection, null, null, null, null, DBHelperItem.COLUMN_NAME_IS_FAVOURITE + order);
        }
        ...
        if (c.moveToPosition(position)) {
            ...
            if(c.getString(c.getColumnIndex(DBHelperItem.COLUMN_NAME_IS_FAVOURITE)).equals("true")){
                item.setFav(true);
            }
            else if(c.getString(c.getColumnIndex(DBHelperItem.COLUMN_NAME_IS_FAVOURITE)).equals("false")){
                item.setFav(false);
            }
            item.setImagePath(c.getString(c.getColumnIndex(DBHelperItem.COLUMN_NAME_IMAGE_PATH)));
    ...
    public long addRecording(String recordingName, String filePath, long length) {
        ...
        cv.put(DBHelperItem.COLUMN_NAME_IS_FAVOURITE, "false");
        cv.put(DBHelperItem.COLUMN_NAME_IMAGE_PATH, "");
    ...
    public void favouriteItem(RecordingItem item, boolean newFavValue, String filePath) {
        SQLiteDatabase db = getWritableDatabase();
        ContentValues cv = new ContentValues();
        if(newFavValue){
            cv.put(DBHelperItem.COLUMN_NAME_IS_FAVOURITE, "true");
        }
        else if(!newFavValue){
            cv.put(DBHelperItem.COLUMN_NAME_IS_FAVOURITE, "false");
        }
    ...
    public void changeImage(RecordingItem item, String imagePath) {
        SQLiteDatabase db = getWritableDatabase();
        ContentValues cv = new ContentValues();
        cv.put(DBHelperItem.COLUMN_NAME_IMAGE_PATH, imagePath);
        db.update(DBHelperItem.TABLE_NAME, cv,
                DBHelperItem._ID + "=" + item.getId(), null);
        if (mOnDatabaseChangedListener != null) {
            mOnDatabaseChangedListener.onDatabaseEntryRenamed();
    ...
    public long restoreRecording(RecordingItem item) {
        ...
        cv.put(DBHelperItem.COLUMN_NAME_IMAGE_PATH, item.getImagePath());
        long rowId = db.insert(DBHelperItem.TABLE_NAME, null, cv);
        if (mOnDatabaseChangedListener != null) {
            //mOnDatabaseChangedListener.onNewDatabaseEntryAdded();
        }
